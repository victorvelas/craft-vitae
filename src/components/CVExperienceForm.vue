...
<template>
  <div class="space-y-6">
    ...
    <p v-if="experiences.length === 0" class="text-xs text-slate-400 italic text-center py-2">
      No experience added yet. Click below to add one.
    </p>
    ...
    <div v-for="(exp, index) in experiences" :key="exp.id" class="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 relative space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Company Name</label>
          <input 
            type="text" 
            v-model="exp.company" 
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Workstation / Position</label>
          <input 
            type="text" 
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Location / Address</label>
        <input 
          type="text" 
          v-model="exp.address" 
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Start Date</label>
          <input 
            type="month" 
        ...
        <div v-show="!exp.current">
          <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">End Date</label>
          <input 
            type="month" 
            v-model="exp.endDate" 
            class="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 transition"
      ...
      <!-- "Current job" radio — single group across all items -->
