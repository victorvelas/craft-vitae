...
<template>
  <div class="space-y-6">
    <!-- Hard Skills -->
    <div class="space-y-2">
      <label class="block text-sm font-semibold text-slate-705 dark:text-slate-300">Hard Skills (Technical)</label>
      <div class="flex gap-2">
        <input 
          type="text" 
          v-model="currentHardSkill" 
          @keydown.enter.prevent="handleAddHardSkill"
          placeholder="e.g. Vue 3, TypeScript, GraphQL (Press Enter)" 
      ...
      <div class="flex flex-wrap gap-1.5 pt-2">
        ...
        >
          ...
          <button 
            type="button"
            @click="removeSkill('hard', index)" 
            class="text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-200 transition-colors cursor-pointer"
          >
        ...
        <p v-if="skills.hard.length === 0" class="text-xs text-slate-400 italic">No technical skills added yet.</p>
      </div>
    </div>

    <!-- Soft Skills -->
    <div class="space-y-2 pt-4 border-t border-slate-100 dark:border-slate-900">
      <label class="block text-sm font-semibold text-slate-705 dark:text-slate-300">Soft Skills (Interpersonal)</label>
      <div class="flex gap-2">
        <input 
          type="text" 
      ...
      <div class="flex flex-wrap gap-1.5 pt-2">
        ...
        <p v-if="skills.soft.length === 0" class="text-xs text-slate-400 italic">No soft skills added yet.</p>
      </div>
    </div>
  </div>
</template>
